...
function start_new_quiz( $quiz_id){

    // $this->check_db_conn();

    //Get new quiz_report_id
    // $sql = "SELECT `AUTO_INCREMENT` FROM INFORMATION_SCHEMA.TABLES WHERE TABLE_SCHEMA = '{$con}' AND TABLE_NAME = 'quiz_report'";
...
//        $objDateTime = new DateTime( 'now', new DateTimeZone("Asia/Kolkata"));
    ...
    // if( $con ){
        $sql = "INSERT INTO `quiz_report`(`id`, `quiz_id`, `start_time`, `user_id`, `is_finished` ) ".
            "VALUES ('{$new_id}', '{$quiz_id}', '{$str_curr_time}', '722', 'N')";
    // } else {
